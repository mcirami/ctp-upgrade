...
class Clicks extends ReportBase
{
    ...
    private function queryEmployee($d_from = false, $d_to = false, $repID, $items_per_page, $offset, $rowCount = false)
    {
    ...
    public function queryAffiliate($d_from = false, $d_to = false, $repID, $items_per_page, $offset, $rowCount = false)
    {
        ...
        if ($per->can("view_fraud_data")) {
            $sql = "SELECT  clicks.idclicks, clicks.first_timestamp,  conversions.timestamp, conversions.paid, click_vars.url,  click_geo.ip,  clicks.offer_idoffer FROM clicks
            ...
            $sql = "SELECT  clicks.first_timestamp, conversions.timestamp,  conversions.paid, click_vars.url,  click_geo.ip, clicks.offer_idoffer  FROM clicks
                    LEFT JOIN click_vars ON click_vars.click_id = clicks.idclicks
                    LEFT JOIN click_geo ON click_geo.click_id = clicks.idclicks
                     LEFT JOIN conversions ON conversions.click_id = clicks.idclicks  
                    WHERE clicks.rep_idrep = :repID ";
